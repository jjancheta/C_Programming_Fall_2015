...
int main(void){
    ...
    for(j=0; j < numDays; j++){                          //getRelevantRecords output =OK
    ...
    for(j=0; j < numDays; j++){                             //sortYearData output = OK
...
void getStats(int month, const struct DailyData yearData[],
   ...
   for (j = startingPoint; j < sz; j++){
        if(month == yearData[j].month){
            ...
            totalDailyAvgTemp += (yearData[j].high + yearData[j].low) / 2;
    ...
    else {                                                    //if no data available for the required year
        monthly->minTemperature = 0;                          // or no data available for certain month/s  
        monthly->maxTemperature = 0;                          // and to prevent printing garbage values.  
